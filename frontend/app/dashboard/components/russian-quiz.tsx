...
interface RussianQuizProps {
    onClose: () => void;
}

export default function RussianQuiz({ onClose }: RussianQuizProps) {
        ...
        const fetchQuestions = async () => {
            ...
            try {
                ...
                const res = await fetch(
                    `${baseUrl}/questions?category=russian&limit=10`,
                );
    ...
    if (isLoading)
        return (
            <div className="text-white text-center py-10 animate-pulse font-medium">
                Вопросы по русскому языку загружаются...
            </div>
        );
    ...
    if (!questions || questions.length === 0)
        return (
            <div className="text-white text-center py-10">
                Rus tili faniga oid savollar topilmadi.
            </div>
        );
