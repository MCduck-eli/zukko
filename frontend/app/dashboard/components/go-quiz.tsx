...
interface GoQuizProps {
    onClose: () => void;
}

export default function GoQuiz({ onClose }: GoQuizProps) {
        ...
        const fetchQuestions = async () => {
            ...
            try {
                ...
                const res = await fetch(
                    `${baseUrl}/questions?category=golang&limit=10`,
                );
    ...
    if (!questions || questions.length === 0)
        return (
            <div className="text-white text-center py-10">
                Go tiliga oid savollar topilmadi.
            </div>
        );
    ...
    if (showResult) {
        ...
        return (
            ...
            >
                <h2 className="text-3xl font-black text-cyan-500 mb-4 uppercase tracking-tighter">
                    NATIJA: {percent}%
                </h2>
                ...
                <button
                    onClick={onClose}
                    className="w-full py-4 bg-cyan-500 text-black font-black rounded-xl hover:bg-cyan-400 transition-colors uppercase tracking-wider"
                >
                    Qayta tanlash
                </button>
            </motion.div>
                ...
                {shuffledOptions.map((opt, i) => {
                    ...
                    return (
                        <motion.button
                            ...
                            className={`w-full p-5 rounded-2xl border ${borderColor} ${bgColor} text-white text-left transition-all flex items-center gap-4 group ${selectedAnswer === null ? "hover:border-cyan-500/30 hover:bg-white/[0.05]" : ""}`}
                        >
                            <span
                                className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-[12px] border transition-colors ${isThisSelected ? "bg-white text-black border-white" : "border-white/10 text-white/40 group-hover:border-cyan-500/30 group-hover:text-cyan-400"}`}
                            >
                                {labels[i]}
                            </span>
